...
def compare_trees(
    ...
    for clade1 in tree1.get_nonterminals():
        ...
        if set(tipNames1) == set(tipNames2):
            continue
            #print("TRUE", tipNames1, tipNames2)
        else:
            try:
                # get parent clades
                parent1=tree1.get_path(clade1)[-2]
                parent2=tree2.get_path(clade2)[-1]
            except IndexError:
    ...
    with open(fileNameALL, "w+") as f:
        ...
        for ii in taxa_conflicting_between_two_trees_tree1:
            if taxa_conflicting_between_two_trees_tree1.index(ii) == 0:
                line="trePlot1 <- ggtree(tre1, branch.length='none') + geom_tiplab() + geom_point2(aes(subset=(node==MRCA(tre1, tip=c("+str(ii).replace("[","").replace("]","")+")))), size=4, color='firebrick2') + "
                f.write(line)
            elif taxa_conflicting_between_two_trees_tree1.index(ii)+1 < len(taxa_conflicting_between_two_trees_tree1):
                line="geom_point2(aes(subset=(node==MRCA(tre1, tip=c("+str(ii).replace("[","").replace("]","")+")))), size=4, color='firebrick2') +"
                f.write(line)
            elif taxa_conflicting_between_two_trees_tree1.index(ii)+1 == len(taxa_conflicting_between_two_trees_tree1):
                line="geom_point2(aes(subset=(node==MRCA(tre1, tip=c("+str(ii).replace("[","").replace("]","")+")))), size=4, color='firebrick2')"
                f.write(line)
                line="+ labs(title='"+tree1Name+"', caption='Red internodes show conflict')\n"
                f.write(line)
        f.write("\n\n# Code for plotting tree2\n")
        # create plot for t2 with geom_points
        for ii in taxa_conflicting_between_two_trees_tree2:
            if taxa_conflicting_between_two_trees_tree2.index(ii) == 0:
                line="trePlot2 <- ggtree(tre2, branch.length='none') + geom_tiplab() + geom_point2(aes(subset=(node==MRCA(tre2, tip=c("+str(ii).replace("[","").replace("]","")+")))), size=4, color='firebrick2') + "
                f.write(line)
            elif taxa_conflicting_between_two_trees_tree2.index(ii)+1 < len(taxa_conflicting_between_two_trees_tree1):
                line="geom_point2(aes(subset=(node==MRCA(tre2, tip=c("+str(ii).replace("[","").replace("]","")+")))), size=4, color='firebrick2') +"
                f.write(line)
            elif taxa_conflicting_between_two_trees_tree2.index(ii)+1 == len(taxa_conflicting_between_two_trees_tree1):
                line="geom_point2(aes(subset=(node==MRCA(tre2, tip=c("+str(ii).replace("[","").replace("]","")+")))), size=4, color='firebrick2')"
                f.write(line)
        ...
        # write plot to figure
        f.write("\n\n# Create plot\n")
        f.write("pdf('tree1_tree2_compare_summary.pdf')\n")
        f.write("plot_grid(trePlot1, trePlot2, ncol=2)\n")
        f.write("dev.off()\n")
